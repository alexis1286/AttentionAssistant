...
public class Observer implements Runnable {
	...
	private boolean currentTaskActive; //used to determine if the task passed is active
	private int observerDelayInterval; //interval when the observer will check on the user's active (IN SECONDS)
	...
	public Observer(Observer observer) {
		...
		this.currentTaskActive = observer.currentTaskActive;
	}
	 ...
	 * @param boolean
	 */
	public void setCurrentTaskStatus(boolean currentTaskActive) {
		this.currentTaskActive= currentTaskActive;
	}
	
	/**
	 * Get time interval for when the observer will check on the user
	 * IN SECONDS
	 * @return int
	 */
	public int getDelayInterval() {
		return this.observerDelayInterval;
	}
	
	/**
	 * Set the time interval for when the observer will check on the user
	 * IN SECONDS
	 * @param int
	 */
	public void setDelayInterval(int observerDelayInterval) {
		this.observerDelayInterval= observerDelayInterval;
	}
	...
	public void run() {
		try {
			this.monitor(this.activeTask, this.db, this.notification_System, this.pTimer);
		} catch (IOException e) {
			System.out.print("!!!Issue with run function in the Observer!!!");
			e.printStackTrace();
		}
	}
	...
	protected void monitor(Task activeTask ,DataBase db, Notification_System notification_System, Pomodoro_Timer pTimer
			...
			this.currentTaskActive = true; //current task is active
			...
			//Starting the threads that running continually
			Thread mouseThread = new Thread(mouseTracker);
			mouseThread.start();
			
			Thread keyboardThread = new Thread(keyBoardTracker);
			...
			while(currentTaskActive) {
				//Delay the while by the observarDelayInterval
				try {
			...
			//closing the trackers that running continually
